...
<script>
    ...
    document.querySelector('input[type="submit"]').addEventListener('click', function (event) {
        ...
        // 선택한 카테고리 값을 숨은 input 필드에 복사하여 전송
        ...
        if (fileList.children.length > 0) {
            ...
            // // fileList에 있는 모든 파일을 FormData에 추가
            // Array.from(fileList.children).forEach(function (fileListItem) {
            //     formData.append('file', fileListItem.file); // fileList의 각 파일을 formData에 추가
            // });
            ...
            xhr.onload = function () {
                if (xhr.status === 200) {
                    // xhr.response 값을 파싱하여 배열로 변환
                    let fileIds = JSON.parse(xhr.response);

                    // 각 파일 ID를 별도의 hidden input으로 추가
                    ...
                    alert('해당 파일을 업로드할 수 없습니다.\n(파일이 없거나 hwp, txt 파일만 가능합니다.)');
                }
                else {
