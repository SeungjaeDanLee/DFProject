    ...
    <div class="Select_all">
        <section>
            ...
            <ul id="existingFileList">
                <c:forEach items="${fileList}" var="file">
                    <li>
                        ...
                        <button type="button" name="updateStatusFile" onclick="updateStatusFile(${file.fno})">삭제</button>
                    </li>
                </c:forEach>
            </ul>
            ...
            <script>
                ...
                <%--        li.innerHTML = `<span>${file.origin_name}</span>`;--%>

                <%--        fileList.appendChild(li);--%>
                <%--    }--%>
                <%--}--%>

                function updateStatusFile(${file.fno}) {
                    // Ajax 요청 생성
                    ...
                    xhr.onload = function () {
                        if (xhr.status === 200) {
                            ...
                            const li = existingFileList.querySelector(`li[data-file-no="${file.fno}"]`);
                            li.parentNode.removeChild(li);
                        } else {
                            // 파일 삭제에 실패했을 때 처리할 코드 작성
                            console.error('파일 삭제에 실패했습니다.');
                            alert('파일 삭제에 실패했습니다.');
                        }
                    };

                    // Ajax 요청 전송
                    xhr.send(`fno=${file.fno}`);
                }
            </script>
