...
//    }
    ...
    // 다중 파일 업로드
    @PostMapping("/fileUpload")
    public ResponseEntity<List<Integer>> uploadFiles(@RequestParam("file") MultipartFile[] files) {
        // 파일 업로드를 위한 서비스 계층 메서드 호출
        List<Integer> uploadedFileIds = new ArrayList<>();
        for (MultipartFile file : files) {
            int fno = fileService.uploadFile(file);
            if (fno != -1) {
                uploadedFileIds.add(fno);
            }
        }

        // 업로드된 파일의 ID 목록을 확인하여 적절한 ResponseEntity를 반환
        if (!uploadedFileIds.isEmpty() && uploadedFileIds.size() <= 5) {
            // 업로드에 성공한 경우
            return ResponseEntity.ok().body(uploadedFileIds);

//            String fileIds = uploadedFileIds.stream().map(Object::toString).collect(Collectors.joining(", "));
...
//    }
    ...
    // 게시글 업데이트 다중 파일 업로드
    @PostMapping("/fileUpdateUpload")
    public ResponseEntity<List<Integer>> uploadUpdateFile(@RequestParam("file") List<MultipartFile> files, @RequestParam("bno") int bno) {
    ...
    public ResponseEntity<byte[]> downloadFile(@RequestParam("file_name") String file_name
                                               ...
                                               ,@RequestParam("origin_name") String origin_name) {
        try {
            // 파일 내용 읽어오기
            ...
            String encodedFileName = URLEncoder.encode(origin_name, StandardCharsets.UTF_8.toString());
    ...
    @PostMapping("/updateStatusFile")
    public ResponseEntity<Integer> updateStatusFile(@RequestParam int fno) {
        int result = fileService.updateStatusFile(fno);
        if (result != -1) {
            return ResponseEntity.ok(result);
        } else {
            return ResponseEntity.noContent().build();
